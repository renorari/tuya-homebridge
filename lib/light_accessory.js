...
// HomeKit color temperature range: 140-500 (mireds)
// Inversion base = min + max = 140 + 500 = 640
const TEMP_INVERSION_BASE = 640;
  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (var statusMap of statusArr) {
      ...
      if (statusMap.code === 'temp_value' || statusMap.code === 'temp_value_v2') {
        ...
        // Invert color temperature if configured
        if (this.platform.config.options && this.platform.config.options.invertTempColor) {
          temperature = TEMP_INVERSION_BASE - temperature;
        }
  ...
  getSendParam(name, value) {
    ...
    switch (name) {
      ...
      case Characteristic.ColorTemperature:
        ...
        // Invert color temperature if configured
        if (this.platform.config.options && this.platform.config.options.invertTempColor) {
          tempValue = TEMP_INVERSION_BASE - value;
        }
        temperature = Math.floor((tempValue - 140) * (this.function_dp_range.temp_range.max - this.function_dp_range.temp_range.min) / 360 + this.function_dp_range.temp_range.min); // value 140~500
